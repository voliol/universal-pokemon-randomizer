...
public class Randomizer {
	...
	public int randomize(final String filename, long seed) {
		...
		if (settings.getTrainersMod() == Settings.TrainersMod.UNCHANGED) {
			...
			romHandler.randomizeTrainerPokes(settings.isTrainersUsePokemonOfSimilarStrength(),
					...
					settings.getTrainersMod() == Settings.TrainersMod.GLOBAL_MAPPING, settings.isGymTypeTheme(),
					settings.isTrainersRandomHeldItem(), settings.isTrainersBuffElite(),
					settings.isTrainersLevelModified() ? settings.getTrainersLevelModifier() : 0);
		}
		...
		if (settings.getPokemonPalettesMod() == Settings.PokemonPalettesMod.RANDOM) {
			romHandler.randomizePokemonPalettes(settings.isPokemonPalettesFollowTypes(),
					settings.isPokemonPalettesFollowEvolutions(), settings.isPokemonPalettesShinyFromNormal());
		}
		...
		if (romHandler.canChangeStarters()) {
			if (settings.getStartersMod() == Settings.StartersMod.CUSTOM) {
				...
				selectRandomStarter(starterCount, starters, typeArr,
						(mustInclude, cannotInclude) -> romHandler.randomStarterPokemon(settings.isStartersNoSplit(),
	...
	private void selectRandomStarter(int starterCount, List<Pokemon> starters, List<Type> typeArr,
		...
		List<Set<Type>> cniList = Arrays.asList(new HashSet<Type>(), new HashSet<Type>(), new HashSet<Type>());
		for (int i = 0; i < starterCount; i++) {
			...
			if (settings.isStartersSETriangle() && settings.isStartersUniqueTypes()) {
				...
				// Pick a pokemon with the type related to this iteration, and excluding any
				// other
				// types
				pkmn = randomPicker.apply(new HashSet<Type>(typeArr.subList(i, i + 1)), cannotInclude);
	...
	private void maybeLogWildPokemonChanges(final RomHandler romHandler) {
		if (settings.getWildPokemonMod() != Settings.WildPokemonMod.UNCHANGED) {
			List<EncounterSet> encounters = romHandler.getEncounters(settings.isUseTimeBasedEncounters()).stream()
					.sorted((x, y) -> x.displayName.compareTo(y.displayName)).collect(Collectors.toList());
			TemplateData.putData("wildPokemon", encounters);
		}
	}

	private void maybeLogTrainerChanges(final RomHandler romHandler) {
		if (settings.getTrainersMod() != Settings.TrainersMod.UNCHANGED || settings.isTrainersForceFullyEvolved()
				|| settings.isTrainersLevelModified() || settings.isRivalCarriesStarterThroughout()
				|| settings.isTrainersRandomHeldItem()) {
			TemplateData.putData("originalTrainers", originalTrainers);
		}
	}

	private int maybeChangeAndLogStaticPokemon(final RomHandler romHandler, boolean raceMode, int checkValue) {
