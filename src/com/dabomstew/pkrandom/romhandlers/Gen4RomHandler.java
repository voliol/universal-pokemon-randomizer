...
public class Gen4RomHandler extends AbstractDSRomHandler {
        ...
        if (romEntry.romType == Gen4Constants.Type_HGSS) {
            ...
            if (tailOffsets.size() == 1) {
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    int color = (i == 0) ? 3 : i;
                    String newStarterDesc = "Professor Elm: " + intros[i] + " \\vFF00\\z000" + color + newStarter.getName()
                            + "\\vFF00\\z0000,\\nthe " + newStarter.getPrimaryType().camelCase() + "-type Pokémon?";
    ...
    private void updatePokedexAreaDataDPPt(List<EncounterSet> encounters) throws IOException {
        ...
        for (EncounterSet es : encounters) {
            if (es.displayName.contains("Mt. Coronet Feebas Tiles")) {
                ...
                for (Encounter enc : es.encounters) {
                    dungeonSpecialPreNationalData[enc.pokemon.getNumber()].add(Gen4Constants.dpptFloaromaMeadowDexIndex);
                    dungeonSpecialPostNationalData[enc.pokemon.getNumber()].add(Gen4Constants.dpptFloaromaMeadowDexIndex);
                    overworldSpecialPreNationalData[enc.pokemon.getNumber()].addAll(Gen4Constants.dpptOverworldHoneyTreeDexIndicies);
                    overworldSpecialPostNationalData[enc.pokemon.getNumber()].addAll(Gen4Constants.dpptOverworldHoneyTreeDexIndicies);
                }
            ...
            for (int i = 0; i < Gen4Constants.hgssHeadbuttOverworldDexMaps.length; i++) {
                ...
                for (Encounter enc : es.encounters) {
                    if (Gen4Constants.hgssHeadbuttOverworldDexMaps[i] != -1) {
                        overworldSpecialData[enc.pokemon.getNumber()].add(Gen4Constants.hgssHeadbuttOverworldDexMaps[i]);
            ...
            if (romEntry.arrayEntries.containsKey("IngameTradePersonTextOffsets")) {
                ...
                for (int trade = 0; trade < textOffsets.length; trade++) {
                    if (textOffsets[trade] > 0) {
                        ...
                        Map<String, String> replacements = new TreeMap<>();
                        replacements.put(oldTrade.givenPokemon.getName().toUpperCase(), newTrade.givenPokemon.getName());
                        if (oldTrade.requestedPokemon != newTrade.requestedPokemon) {
                            replacements.put(oldTrade.requestedPokemon.getName().toUpperCase(), newTrade.requestedPokemon.getName());
                        }
    ...
    public List<Integer> getSensibleHeldItemsFor(TrainerPokemon tp, boolean consumableOnly, List<Move> moves, int[] pokeMoves) {
        ...
        Map<Type, Effectiveness> byType = Effectiveness.against(tp.pokemon.getPrimaryType(), tp.pokemon.getSecondaryType(), 4, effectivenessUpdated);
