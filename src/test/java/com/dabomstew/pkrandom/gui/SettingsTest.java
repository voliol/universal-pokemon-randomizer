...
public class SettingsTest extends AbstractUIBase {
    ...
    public void TestUseResistantType() throws IOException {
        ...
        checkboxEnabledAndSelected(resistantTypeCBFixture, (setts) -> (setts.getCurrentMiscTweaks()
                & MiscTweak.USE_RESISTANT_TYPE.getValue()) > 0);
     ...
     * 
     * Selecting "UNCHANGED" or "TYPE THEME" disables the Gym Type Theme checkbox and resets state
     * to false
     * 
     * Verifies settings can be stored and loaded with no error and preserve state
     * 
    ...
    public void TestRandomSettingsButton() throws IOException {
        ...
        assertTrue("Settings do not match after reload - \nBefore: " + settingsString + "\nAfter: "
                + settings.toString(), settingsString.equals(settings.toString()));
    }
    ...
    public void TestMinimumCatchRateCB() throws IOException {
        ...
        assertTrue(
                "Wild Pokemon should be set to " + unchangedWildPokemonRBFixture.text()
                        + " but was not",
        ...
        checkboxEnabledButNotSelected(minimumCatchRateCBFixture,
                (setts) -> setts.isUseMinimumCatchRate());
        assertTrue(
                "Wild Pokemon was not " + unchangedWildPokemonRBFixture.text()
        ...
        checkboxEnabledAndSelected(minimumCatchRateCBFixture,
                (setts) -> setts.isUseMinimumCatchRate());
        assertTrue(
                "Wild Pokemon should be set to " + randomWildPokemonRBFixture.text()
                        + " but was not",
        ...
        checkboxEnabledAndSelected(minimumCatchRateCBFixture,
                (setts) -> setts.isUseMinimumCatchRate());
        assertTrue(
                "Wild Pokemon was not " + randomWildPokemonRBFixture.text()
                        + " after reloading settings " + settingsReloadCount,
                settings.getWildPokemonMod() == Settings.WildPokemonMod.RANDOM);
        settingsReloadCount++;
    ...
    public void TestStarterTypeFilter() throws IOException {
        ...
        String settingsString = settings.toString();
        settings = Settings.fromString(settingsString);
        assertTrue("Starter Types was not null after reloading settings " + settingsReloadCount,
        ...
        settings = Settings.fromString(settingsString);
        assertTrue(
                "Starter Types did not include NORMAL and DARK after reloading settings "
                        + settingsReloadCount,
                settings.getStarterTypes().containsAll(Arrays.asList(Type.NORMAL, Type.DARK)));
        assertTrue("Starters was not RANDOM after reloading settings " + settingsReloadCount,
                settings.getStartersMod() == Settings.StartersMod.RANDOM);
        ...
        Type.getTypes(typeSize)
                .forEach(type -> assertTrue("Missing " + type + " from starter types",
                        starterTypes1.contains(type)));
        settingsString = settings.toString();
        settings = Settings.fromString(settingsString);
        List<Type> starterTypes2 = settings.getStarterTypes();
        Type.getTypes(typeSize)
                .forEach(type -> assertTrue("Missing " + type + " from starter types after reload",
    ...
    public void TestTrainersBuffElite() throws IOException {
        ...
        TestCheckboxBasedOnRadioButton(unchangedTrainerRBFixture, globalSwapTrainerRBFixture,
                buffEliteCBFixture, (settings) -> settings.isTrainersBuffElite(),
                (trainersMod) -> trainersMod == Settings.TrainersMod.UNCHANGED,
                (trainersMod) -> trainersMod == Settings.TrainersMod.GLOBAL_MAPPING,
                (settings) -> settings.getTrainersMod(), "Trainers");
    ...
    public void TestEvolveLevel1() throws IOException {
        JRadioButtonFixture unchangedEvolveRBFixture = getRadioButtonByName("peUnchangedRB");
     ...
     * @param settingsCheckboxFunction - The method in Settings.java that refers to the state of the
     *            checkbox
     * @param defaultRBCondition - The enum value that represents the defaultRb in radio button
     *            group
     * @param triggerRBCondition - The enum value that represents the triggerRB in the radio button
     *            group
    ...
    private void TestCheckboxBasedOnRadioButton(JRadioButtonFixture defaultRB,
            ...
            Predicate<Settings> settingsCheckboxFunction, Predicate<Enum> defaultRBCondition,
            Predicate<Enum> triggerRBCondition, Function<Settings, Enum> settingsRBFunction,
            String buttonGroup) throws IOException {
        int settingsReloadCount = 0;
        // Sanity check - should evaluate to false
        Settings settings = this.mainWindow.getCurrentSettings();
        assertFalse(checkboxToTest.text() + " should not be set yet",
                settingsCheckboxFunction.test(settings));
        assertFalse(checkboxToTest.text() + " should not be enabled yet",
                checkboxToTest.isEnabled());
        assertTrue(checkboxToTest.text() + " should not be selected yet",
                checkboxToTest.requireNotSelected() == checkboxToTest);
        assertTrue(buttonGroup + " should be set to " + defaultRB.text() + " but was not",
                defaultRBCondition.test(settingsRBFunction.apply(settings)));
        // Sanity check - Should not fail with 0 options
        String setttingsString = settings.toString();
        settings = Settings.fromString(setttingsString);
        assertFalse(checkboxToTest.text() + " was not false after reloading settings "
                + settingsReloadCount, settingsCheckboxFunction.test(settings));
        assertFalse(checkboxToTest.text() + " should not be enabled after reloading settings "
                + settingsReloadCount, checkboxToTest.isEnabled());
        assertTrue(
                checkboxToTest.text() + " should not be selected after reloading settings "
                        + settingsReloadCount,
                checkboxToTest.requireNotSelected() == checkboxToTest);
        assertTrue(
                buttonGroup + " was not " + defaultRB.text() + " after reloading settings "
                        + settingsReloadCount,
                defaultRBCondition.test(settingsRBFunction.apply(settings)));
        settingsReloadCount++;
        ...
        assertTrue(checkboxToTest.text() + " should not be selected yet",
                checkboxToTest.requireNotSelected() == checkboxToTest);
        assertTrue(buttonGroup + " should be set to " + triggerRB.text() + " but was not",
                triggerRBCondition.test(settingsRBFunction.apply(settings)));
        setttingsString = settings.toString();
        settings = Settings.fromString(setttingsString);
        assertFalse(checkboxToTest.text() + " was not false after reloading settings "
                + settingsReloadCount, settingsCheckboxFunction.test(settings));
        assertTrue(checkboxToTest.text() + " should be enabled after reloading settings "
                + settingsReloadCount, checkboxToTest.isEnabled());
        assertTrue(
                checkboxToTest.text() + " should not be selected after reloading settings "
                        + settingsReloadCount,
                checkboxToTest.requireNotSelected() == checkboxToTest);
        assertTrue(
                buttonGroup + " was not " + triggerRB.text() + " after reloading settings "
                        + settingsReloadCount,
                triggerRBCondition.test(settingsRBFunction.apply(settings)));
        settingsReloadCount++;
        ...
        settings = this.mainWindow.getCurrentSettings();
        assertTrue(checkboxToTest.text() + " should be set now",
                settingsCheckboxFunction.test(settings));
        assertTrue(checkboxToTest.text() + " should be enabled as state did not change",
                checkboxToTest.isEnabled());
        assertTrue(checkboxToTest.text() + " should  be selected now",
                checkboxToTest.requireSelected() == checkboxToTest);
        assertTrue(
                buttonGroup + " should be set to " + triggerRB.text() + " as state did not change",
                triggerRBCondition.test(settingsRBFunction.apply(settings)));
        setttingsString = settings.toString();
        settings = Settings.fromString(setttingsString);
        assertTrue(checkboxToTest.text() + " was not true after reloading settings "
                + settingsReloadCount, settingsCheckboxFunction.test(settings));
        assertTrue(checkboxToTest.text() + " should be enabled after reloading settings "
                + settingsReloadCount, checkboxToTest.isEnabled());
        assertTrue(checkboxToTest.text() + " should  be selected after reloading settings "
                + settingsReloadCount, checkboxToTest.requireSelected() == checkboxToTest);
        assertTrue(
                buttonGroup + " was not " + triggerRB.text() + " after reloading settings "
                        + settingsReloadCount,
                triggerRBCondition.test(settingsRBFunction.apply(settings)));
        ...
        checkboxToTest.requireVisible().requireEnabled().click();
        settings = this.mainWindow.getCurrentSettings();
        assertFalse(checkboxToTest.text() + " should be unset now",
                settingsCheckboxFunction.test(settings));
        assertTrue(checkboxToTest.text() + " should be enabled as state did not change",
                checkboxToTest.isEnabled());
        assertTrue(checkboxToTest.text() + " should not be selected now",
                checkboxToTest.requireNotSelected() == checkboxToTest);
        assertTrue(
                buttonGroup + " should be set to " + triggerRB.text() + " as state did not change",
                triggerRBCondition.test(settingsRBFunction.apply(settings)));
     ...
     * @param defaultRBCondition - The enum value that represents the defaultRb in radio button
     *            group
     * @param settingsRBCondition - The enum value that represents the radioButtonToTest in radio
     *            button group
     * @param settingsRBFunction - The method in Settings.java that refers to the state of the enum
     * @param buttonGroup - The name of the radio button group. Used for descriptive error messages
     * @throws IOException
    ...
    private void TestRadioButton(JRadioButtonFixture defaultRB,
            ...
            Predicate<Enum> settingsRBCondition, Function<Settings, Enum> settingsRBFunction,
            String buttonGroup) throws IOException {
        int settingsReloadCount = 0;
        // Sanity check - should evaluate to false
        Settings settings = this.mainWindow.getCurrentSettings();
        assertTrue(radioButtonToTest.text() + " should be false",
                radioButtonToTest.requireNotSelected() == radioButtonToTest);
        assertTrue(buttonGroup + " should be set to " + defaultRB.text() + " but was not",
                defaultRBCondition.test(settingsRBFunction.apply(settings)));
        // Sanity check - Should not fail with 0 options
        String setttingsString = settings.toString();
        settings = Settings.fromString(setttingsString);
        assertTrue(
                radioButtonToTest.text() + " was not false after reloading settings "
                        + settingsReloadCount,
                radioButtonToTest.requireNotSelected() == radioButtonToTest);
        assertTrue(
                buttonGroup + " was not " + defaultRB.text() + " after reloading settings "
        ...
        settings = this.mainWindow.getCurrentSettings();
        assertTrue(radioButtonToTest.text() + " should be true",
                radioButtonToTest.requireSelected() == radioButtonToTest);
        assertTrue(buttonGroup + " should be set to " + radioButtonToTest.text() + " but was not",
                settingsRBCondition.test(settingsRBFunction.apply(settings)));
        // Reloading settings
        setttingsString = settings.toString();
        settings = Settings.fromString(setttingsString);
        assertTrue(
                radioButtonToTest.text() + " was not true after reloading settings "
                        + settingsReloadCount,
                radioButtonToTest.requireSelected() == radioButtonToTest);
        assertTrue(
                buttonGroup + " was not " + radioButtonToTest.text() + " after reloading settings "
                        + settingsReloadCount,
                settingsRBCondition.test(settingsRBFunction.apply(settings)));
        settingsReloadCount++;
    ...
    /**
     * Checks if a checkbox is both disabled and unselected, and confirms the settings state is also
     * false
     * 
    ...
    private void checkboxNotEnabledAndNotSelected(JCheckBoxFixture cbFixture,
            Predicate<Settings> settingsCheckboxFunction) throws IOException {
        Settings settings = this.mainWindow.getCurrentSettings();
