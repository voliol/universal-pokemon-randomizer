...
public class Gen5Test {
        ...
        /**
         * When altering starters, make sure the ROM's text is updated when it is a Black 2/White 2
         * ROM
         * 
         * @throws IOException
         */
        ...
        public void TestBW1NacreneCityText() throws IOException {
                ...
                romhandler.randomizeTrainerPokes(false, false, false, false, false, false, false,
                                true, false, false, 0);
                verify(mockTextHandler, times(1)).bw1NacreneCityTextModifications(isNotNull(),
                                any());
        }
        ...
        public void TestBW1CasteliaCityPraiseText() throws IOException {
                ...
                romhandler.randomizeTrainerPokes(false, false, false, false, false, false, false,
                                true, false, false, 0);
                verify(mockTextHandler, times(1))
                                .bw1CasteliaCityPraiseTextModifications(isNotNull());
        }
        ...
        public void TestBW1CasteliaCityBurghText() throws IOException {
                ...
                romhandler.randomizeTrainerPokes(false, false, false, false, false, false, false,
                                true, false, false, 0);
                verify(mockTextHandler, times(1))
                                .bw1CasteliaCityBurghTextModifications(isNotNull());
        }
        ...
        public void TestBW1NimbasaCityText() throws IOException {
                ...
                romhandler.randomizeTrainerPokes(false, false, false, false, false, false, false,
                                true, false, false, 0);
                verify(mockTextHandler, times(1)).bw1NimbasaCityTextModifications(isNotNull(),
                                any());
        }
        ...
        public void TestBW1DriftveilCityText() throws IOException {
                ...
                romhandler.randomizeTrainerPokes(false, false, false, false, false, false, false,
                                false, false, false, 0);
                verify(mockTextHandler, times(1)).bw1DriftveilCityTextModifications(isNull(),
                                any());

                // Verify with type theming
                romhandler.randomizeTrainerPokes(false, false, false, false, false, false, false,
                                true, false, false, 0);
                verify(mockTextHandler, times(1)).bw1DriftveilCityTextModifications(isNotNull(),
                                any());
        }
        ...
        public void TestGen5ChangeMethods() throws IOException {
                ...
                romhandler.getMainPokemonList().forEach(pk -> {
                        ...
                        HashSet<EvolutionType> uniqueMethods =
                                        new HashSet<EvolutionType>(usedMethods);
                        assertTrue("Duplicate method detected - "
                                        + Arrays.toString(usedMethods.toArray()),
                                        uniqueMethods.size() == usedMethods.size());
                        HashSet<Integer> uniqueStones = new HashSet<Integer>(usedStones);
                        assertTrue("Duplicate stone detected - "
                                        + Arrays.toString(usedStones.toArray()),
                                        uniqueStones.size() == usedStones.size());
                        HashSet<Integer> uniqueItems = new HashSet<Integer>(usedItems);
                        assertTrue("Duplicate item detected - "
                                        + Arrays.toString(usedItems.toArray()),
                                        uniqueItems.size() == usedItems.size());
                        HashSet<Integer> uniquePokemon = new HashSet<Integer>(usedPokemon);
                        assertTrue("Duplicate pokemon detected - "
                                        + Arrays.toString(usedPokemon.toArray()),
                                        uniquePokemon.size() == usedPokemon.size());
                        HashSet<Integer> uniqueMoves = new HashSet<Integer>(usedMoves);
                        assertTrue("Duplicate move detected - "
                                        + Arrays.toString(usedMoves.toArray()),
                ...
                romhandler.getMainPokemonList().forEach(pk -> {
                        ...
                        pk.evolutionsFrom.forEach(evo -> {
                                ...
                                if (EvolutionType.isOfType("Stone", evo.type)) {
                                        ...
                                        usedItems.add(evo.extraInfo);
                                        ArrayList<EvolutionType> etList = itemEvos.getOrDefault(
                                                        evo.extraInfo,
                        ...
                        // - The only acceptable occurrence is if there is exactly 1
                        // LEVEL_ITEM_NIGHT and
                        ...
                        if (uniqueItems.size() != usedItems.size()) {
                                ...
                                for (Integer j : dupsToCheck) {
                                        ...
                                        // Cannot be more than length 2 and must contain
                                        // LEVEL_ITEM_DAY and LEVEL_ITEM_NIGHT
                                        assertTrue("Duplicate items detected - Item " + j
                                                        + " is used by "
                                                        + Arrays.toString(typeWithItem.toArray()),
                                                        typeWithItem.size() == 2 && typeWithItem
                                                                        .contains(EvolutionType.LEVEL_ITEM_DAY)
                                                                        && typeWithItem.contains(
         ...
         */
        @Test
        public void TestGen5UpdateExtraInfoRemovesDuplicates() throws IOException {
                ...
                pokemonList.get(0).evolutionsFrom.add(ev);
                romhandler.updateExtraInfo(ev);
                assertTrue("Level with other expected 1 but got " + ev.extraInfo,
                                ev.extraInfo == 1);
                ...
                for (Trainer t : romhandler.getTrainers()) {
                        for (TrainerPokemon tp : t.getPokemon()) {
                                assertTrue(tp.heldItem + " was not in Gen 5 allowed items.",
                                                Gen5Constants.trainerItemList
                                                                .isAllowed(tp.heldItem));
                        }
                }
        }
        ...
        private void setUp() {
                ...
                for (int i = 0; i < Gen5Constants.pokemonCount + 1; i++) {
                        ...
                        pokemonList.add(pk);
                        for (int j = 0; j < i % 3; j++) {
                                Evolution ev = new Evolution(pk, new Pokemon(), false,
                                                EvolutionType.LEVEL, 1);
